...
exports.getAttendanceHistory = exports.getTodayStatus = exports.checkOut = exports.checkIn = void 0;
const database_js_1 = __importDefault(require("../config/database.js"));
const haversine_js_1 = require("../utils/haversine.js");
const insights_service_js_1 = require("../services/insights.service.js");
/**
 * Helper to get current office configuration settings or use defaults
 */
const getOfficeSettings = async () => {
    const result = await database_js_1.default.query('SELECT * FROM office_settings LIMIT 1');
...
const checkIn = async (req, res) => {
    ...
    try {
        ...
        const existingCheckIn = await database_js_1.default.query('SELECT id FROM attendance WHERE user_id = $1 AND date = $2', [user.id, todayStr]);
        if (existingCheckIn.rows.length > 0) {
         ...
         VALUES ($1, 'CHECK_IN_FAIL', $2, $3, 0, false, 'Duplicate check-in attempt on the same day')`, [user.id, latitude, longitude]);
        ...
        const distance = (0, haversine_js_1.calculateDistance)(latitude, longitude, parseFloat(office.latitude), parseFloat(office.longitude));
        if (distance > parseFloat(office.radius_meters)) {
            ...
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_IN_FAIL', $2, $3, $4, false, $5)`, [user.id, latitude, longitude, distance, reason]);
            res.status(400).json({
        ...
        const attendanceResult = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, is_late, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [user.id, todayStr, now, latitude, longitude, isLate, status]);
        const attendanceId = attendanceResult.rows[0].id;
        // 6. Register successful log in audit trail
        await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, lat, lng, distance_from_office, accepted, reason)
       VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, true, 'Check-in within geofence boundaries')`, [user.id, latitude, longitude, distance]);
        // 7. Run real-time anomaly analysis on check-in telemetry
        await (0, insights_service_js_1.analyzeAttendanceSession)(user.id, attendanceId, latitude, longitude, now, user.shift_start_time);
        res.status(201).json({
...
const checkOut = async (req, res) => {
    ...
    try {
        ...
        const attendanceRecord = await database_js_1.default.query('SELECT * FROM attendance WHERE user_id = $1 AND date = $2', [user.id, todayStr]);
        if (attendanceRecord.rows.length === 0) {
            // Log the failed sequence attempt
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_OUT_FAIL', $2, $3, 0, false, 'Check-out attempted without a prior check-in')`, [user.id, latitude, longitude]);
        ...
        if (attendance.check_out_time) {
            ...
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_OUT_FAIL', $2, $3, 0, false, 'Duplicate check-out attempt on the same day')`, [user.id, latitude, longitude]);
        ...
        if (distance > parseFloat(office.radius_meters)) {
            ...
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_OUT_FAIL', $2, $3, $4, false, $5)`, [user.id, latitude, longitude, distance, reason]);
            res.status(400).json({
        ...
        const updateResult = await database_js_1.default.query(`UPDATE attendance
       SET check_out_time = $1, check_out_lat = $2, check_out_lng = $3, working_hours = $4, updated_at = CURRENT_TIMESTAMP
       WHERE id = $5
       RETURNING *`, [now, latitude, longitude, workingHours, attendance.id]);
        // 6. Register successful check-out log in audit trail
        await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, lat, lng, distance_from_office, accepted, reason)
       VALUES ($1, 'CHECK_OUT_SUCCESS', $2, $3, $4, true, 'Check-out within geofence boundaries')`, [user.id, latitude, longitude, distance]);
        res.status(200).json({
...
const getTodayStatus = async (req, res) => {
    ...
    try {
        const todayStr = new Date().toISOString().split('T')[0];
        const result = await database_js_1.default.query('SELECT * FROM attendance WHERE user_id = $1 AND date = $2', [user.id, todayStr]);
        if (result.rows.length === 0) {
            res.status(200).json({
                hasCheckedIn: false,
                hasCheckedOut: false,
