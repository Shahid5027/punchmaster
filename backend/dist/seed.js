...
const seedDatabase = async () => {
    ...
    try {
        ...
        await database_js_1.default.query('BEGIN');
        // Create users table
    ...
    `);
        ...
        await database_js_1.default.query('CREATE INDEX IF NOT EXISTS idx_users_email ON users(email);');
    ...
    `);
        ...
        await database_js_1.default.query('CREATE INDEX IF NOT EXISTS idx_logs_user_timestamp ON attendance_logs(user_id, timestamp DESC);');
    ...
    `);
        ...
        await database_js_1.default.query('CREATE INDEX IF NOT EXISTS idx_insights_user ON attendance_insights(user_id);');
        // Clean out existing seeded data so we get a completely fresh, high-fidelity experience
        await database_js_1.default.query('TRUNCATE office_settings, attendance_insights, attendance_logs, attendance, users CASCADE;');
    ...
    `);
        ...
        const employeeHash = await bcryptjs_1.default.hash('employee123', 10);
        const employees = [
        ...
        // Seed admin
        const adminRes = await database_js_1.default.query(`INSERT INTO users (name, email, password_hash, role, shift_start_time)
       VALUES ($1, $2, $3, 'admin', '09:00:00') RETURNING id`, ['System Admin', 'admin@geoshield.ai', adminHash]);
        console.log('👤 Seeded Admin: admin@geoshield.ai');
        // Seed employees and map their IDs
        const empIds = {};
        for (const emp of employees) {
            const res = await database_js_1.default.query(`INSERT INTO users (name, email, password_hash, role, shift_start_time)
         VALUES ($1, $2, $3, 'employee', $4) RETURNING id`, [emp.name, emp.email, employeeHash, emp.shift]);
        ...
        for (let i = 1; i <= 15; i++) {
            ...
            if (datesToSeed.length >= 10)
                break; // keep historical range to 10 solid days
        }
        ...
        for (const date of datesToSeed) {
            // Alex Mercer (Perfect attendance, on-time)
            {
                ...
                const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, check_out_time, check_out_lat, check_out_lng, working_hours, is_late, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 8.2, false, 'PRESENT') RETURNING id`, [userId, date, checkInTime, coords.lat, coords.lng, checkOutTime, coords.lat, coords.lng]);
                const attId = att.rows[0].id;
                await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
           VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.4, true, 'Location verified in-zone')`, [userId, checkInTime, coords.lat, coords.lng]);
                await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
           VALUES ($1, $2, 98, 'Normal', 0, 'On-time verification inside corporate geofence')`, [attId, userId]);
            }
            // Sarah Connor (Custom shift 08:30, occasionally late)
            {
                ...
                const coords = getInZoneCoords();
                const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, check_out_time, check_out_lat, check_out_lng, working_hours, is_late, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 8.5, $9, $10) RETURNING id`, [userId, date, checkInTime, coords.lat, coords.lng, checkOutTime, coords.lat, coords.lng, isLate, isLate ? 'LATE' : 'PRESENT']);
                const attId = att.rows[0].id;
                await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
           VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.8, true, 'Location verified in-zone')`, [userId, checkInTime, coords.lat, coords.lng]);
                await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
           VALUES ($1, $2, 96, 'Normal', 0, $3)`, [attId, userId, isLate ? 'Slightly late arrival, geofence normal' : 'On-time shift start']);
            }
            // Marcus Wright (Checked in but with coordinate drift / location mismatches)
            {
                ...
                const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, check_out_time, check_out_lat, check_out_lng, working_hours, is_late, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 8.0, false, 'PRESENT') RETURNING id`, [userId, date, checkInTime, lat, lng, checkOutTime, lat, lng]);
                const attId = att.rows[0].id;
                await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
           VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, $5, true, 'Location within geofence outer ring')`, [userId, checkInTime, lat, lng, dist]);
                await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, unusual_location_detected, details)
           VALUES ($1, $2, $3, $4, 0, $5, $6)`, [attId, userId, score, status, hasDrift, details]);
            }
            // David Lightman (Pre-filled standard history, but has 2 out-of-zone attempts in logs on one day)
            {
                ...
                const coords = getInZoneCoords();
                const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, check_out_time, check_out_lat, check_out_lng, working_hours, is_late, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 8.1, false, 'PRESENT') RETURNING id`, [userId, date, checkInTime, coords.lat, coords.lng, checkOutTime, coords.lat, coords.lng]);
                const attId = att.rows[0].id;
                await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
           VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.2, true, 'Verified corporate HQ geofence')`, [userId, checkInTime, coords.lat, coords.lng]);
                await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
           VALUES ($1, $2, 97, 'Normal', 0, 'On-time verification')`, [attId, userId]);
                // Seed 1 historical out-of-zone attempt for David on an older date to show up in logs
                if (date === datesToSeed[2]) {
                    await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
             VALUES ($1, 'CHECK_IN_FAIL', $2, 12.985200, 77.604200, 1540.0, false, 'Check-in blocked: Coordinates out-of-zone')`, [userId, `${date}T08:42:15Z`]);
                }
            }
            // John Connor (Tardiness pattern, frequently late)
            {
                ...
                const coords = getInZoneCoords();
                const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, check_out_time, check_out_lat, check_out_lng, working_hours, is_late, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 7.8, $9, $10) RETURNING id`, [userId, date, checkInTime, coords.lat, coords.lng, checkOutTime, coords.lat, coords.lng, isLate, isLate ? 'LATE' : 'PRESENT']);
                const attId = att.rows[0].id;
                await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
           VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.3, true, 'Verified corporate HQ geofence')`, [userId, checkInTime, coords.lat, coords.lng]);
                await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
           VALUES ($1, $2, 97, 'Normal', 0, $3)`, [attId, userId, isLate ? 'Late arrival recorded' : 'Normal']);
            }
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 8.2, false, 'PRESENT') RETURNING id`, [userId, date, checkInTime, coords.lat, coords.lng, checkOutTime, coords.lat, coords.lng]);
                const attId = att.rows[0].id;
                await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
           VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 0.9, true, 'Verified corporate HQ geofence')`, [userId, checkInTime, coords.lat, coords.lng]);
                await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
           VALUES ($1, $2, 99, 'Normal', 0, 'Exemplary on-time record')`, [attId, userId]);
            }
        }
        ...
        // Alex Mercer: Successful check-in today
        {
            ...
            const coords = getInZoneCoords();
            const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, status)
         VALUES ($1, $2, $3, $4, $5, 'PRESENT') RETURNING id`, [userId, todayStr, checkInTime, coords.lat, coords.lng]);
            const attId = att.rows[0].id;
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.2, true, 'Verified corporate HQ geofence')`, [userId, checkInTime, coords.lat, coords.lng]);
            await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
         VALUES ($1, $2, 98, 'Normal', 0, 'On-time dynamic check-in')`, [attId, userId]);
        ...
        // Ellen Ripley: Successful check-in today
        {
         ...
         VALUES ($1, $2, $3, $4, $5, 'PRESENT') RETURNING id`, [userId, todayStr, checkInTime, coords.lat, coords.lng]);
            const attId = att.rows[0].id;
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.1, true, 'Verified corporate HQ geofence')`, [userId, checkInTime, coords.lat, coords.lng]);
            await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
         VALUES ($1, $2, 99, 'Normal', 0, 'Perfect dynamic check-in')`, [attId, userId]);
        ...
        // Marcus Wright: Checked in today but has COORDINATE DRIFT (Location mismatch)
        {
            ...
            const dist = 184.2;
            const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, status)
         VALUES ($1, $2, $3, $4, $5, 'PRESENT') RETURNING id`, [userId, todayStr, checkInTime, lat, lng]);
            const attId = att.rows[0].id;
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, $5, true, 'Verified check-in inside outer ring')`, [userId, checkInTime, lat, lng, dist]);
            await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, unusual_location_detected, details)
         VALUES ($1, $2, 45, 'Unusual location', 0, true, 'Check-in coordinates drifted 184.2m from Bangalore office boundary.')`, [attId, userId]);
        ...
        // John Connor: Checked in late today
        {
            ...
            const coords = getInZoneCoords();
            const att = await database_js_1.default.query(`INSERT INTO attendance (user_id, date, check_in_time, check_in_lat, check_in_lng, is_late, status)
         VALUES ($1, $2, $3, $4, $5, true, 'LATE') RETURNING id`, [userId, todayStr, checkInTime, coords.lat, coords.lng]);
            const attId = att.rows[0].id;
            await database_js_1.default.query(`INSERT INTO attendance_logs (user_id, event_type, timestamp, lat, lng, distance_from_office, accepted, reason)
         VALUES ($1, 'CHECK_IN_SUCCESS', $2, $3, $4, 1.5, true, 'Location verified in-zone')`, [userId, checkInTime, coords.lat, coords.lng]);
            await database_js_1.default.query(`INSERT INTO attendance_insights (attendance_id, user_id, confidence_score, confidence_status, failed_checkin_count, details)
         VALUES ($1, $2, 95, 'Normal', 0, 'Late check-in dynamically recorded')`, [attId, userId]);
        ...
        // David Lightman: 2 Repeated Out-of-Zone Attempts today (absent on active roster)
        {
         ...
         VALUES ($1, 'CHECK_IN_FAIL', $2, 12.981000, 77.602500, 1350.0, false, 'Check-in blocked: Coordinates out-of-zone'),
                ($1, 'CHECK_IN_FAIL', $3, 12.980500, 77.602200, 1290.0, false, 'Check-in blocked: Coordinates out-of-zone')`, [userId, `${todayStr}T08:45:12Z`, `${todayStr}T08:55:40Z`]);
        ...
        await database_js_1.default.query('COMMIT');
        console.log('✅ Workforce Analytics database seeding completed successfully!');
    }
    catch (error) {
        await database_js_1.default.query('ROLLBACK');
