...
exports.updateOfficeConfig = exports.getOfficeConfig = void 0;
const database_js_1 = __importDefault(require("../config/database.js"));
/**
 * Fetch Office Geofencing configuration
 */
const getOfficeConfig = async (req, res) => {
    if (!req.user) {
        res.status(401).json({ error: 'Unauthorized' });
        return;
    }
    try {
        const result = await database_js_1.default.query('SELECT latitude, longitude, radius_meters, late_threshold_minutes FROM office_settings LIMIT 1');
 ...
 */
const updateOfficeConfig = async (req, res) => {
    ...
    try {
        ...
        const checkExist = await database_js_1.default.query('SELECT id FROM office_settings LIMIT 1');
        if (checkExist.rows.length === 0) {
            // Insert new settings row
            await database_js_1.default.query(`INSERT INTO office_settings (latitude, longitude, radius_meters, late_threshold_minutes)
         VALUES ($1, $2, $3, $4)`, [latNum, lngNum, radNum, thresholdNum]);
        }
        else {
            // Update existing settings row
            await database_js_1.default.query(`UPDATE office_settings 
         SET latitude = $1, longitude = $2, radius_meters = $3, late_threshold_minutes = $4, updated_at = CURRENT_TIMESTAMP`, [latNum, lngNum, radNum, thresholdNum]);
        }
        res.status(200).json({
...
exports.updateOfficeConfig = updateOfficeConfig;
